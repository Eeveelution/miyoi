...
namespace
{
eastl::pair<uint16_t, uint16_t> getXY16(uint32_t xy)
{
...
TimFile readTimFile(const eastl::vector<uint8_t>& data)
{
    ...
    if (tim.hasClut) { // CLUT
        ...
        for (auto& clut : tim.cluts) {
            ...
            fr.GetBytes(clut.colors.data(), clutNumColors * sizeof(uint16_t));
        }
    }
    ...
    // pixel data
    {
        ...
        if (tim.hasClut) {
            ...
            fr.GetBytes(tim.pixelsIdx.data(), numPixels * sizeof(uint8_t));
        } else {
        ...
        for (int i = 0; i < pixW * pixH; ++i) {
            ...
            if (tim.pmode == TimFile::PMode::Clut4Bit) {
                tim.pixelsIdx[i * 4 + 0] = static_cast<uint8_t>((pd & 0x000F) >> 0);
                tim.pixelsIdx[i * 4 + 1] = static_cast<uint8_t>((pd & 0x00F0) >> 4);
                tim.pixelsIdx[i * 4 + 2] = static_cast<uint8_t>((pd & 0x0F00) >> 8);
                tim.pixelsIdx[i * 4 + 3] = static_cast<uint8_t>((pd & 0xF000) >> 12);
            } else if (tim.pmode == TimFile::PMode::Clut8Bit) {
                tim.pixelsIdx[i * 2 + 0] = static_cast<uint8_t>(pd);
                tim.pixelsIdx[i * 2 + 1] = static_cast<uint8_t>(pd >> 8);
            } else if (tim.pmode == TimFile::PMode::Direct15Bit) {
                tim.pixels[i] = pd;
            }
        } 
